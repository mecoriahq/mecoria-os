...
TARGET_ENV_KEY = "NOTION_PUBLISHING_QUEUE_DB_ID"
TARGET_KEY = "publishing_queue"

DESIRED_PROPERTIES = {
    "Channel Key": {"rich_text": {}},
    "Video Title": {"rich_text": {}},
    "Queue Status": {"rich_text": {}},
    "Release Version": {"rich_text": {}},
    "Public URL": {"url": {}},
    "Unlisted URL": {"url": {}},
    "Visibility": {"rich_text": {}},
    "Public Status": {"rich_text": {}},
    "Upload Ready": {"checkbox": {}},
    "First Public Video": {"checkbox": {}},
    "Next Step": {"rich_text": {}},
    "Source Record": {"rich_text": {}},
    "Video File Path": {"rich_text": {}},
    "Thumbnail Image Path": {"rich_text": {}},
    "Sync Source": {"rich_text": {}},
    "System Key": {"rich_text": {}}
}

FIELD_CANDIDATES = {
    "channel_key": ["Channel Key", "Channel"],
    "video_title": ["Video Title", "Title"],
    "queue_status": ["Queue Status", "Publishing Status", "Status"],
    "release_version": ["Release Version", "Version"],
    "public_url": ["Public URL", "Public Video URL"],
    "unlisted_url": ["Unlisted URL", "Unlisted Video URL"],
    "visibility": ["Visibility"],
    "public_status": ["Public Status"],
    "upload_ready": ["Upload Ready"],
    "first_public_video": ["First Public Video"],
    "next_step": ["Next Step"],
    "source_record": ["Source Record"],
    "video_file_path": ["Video File Path"],
    "thumbnail_image_path": ["Thumbnail Image Path"],
...
def load_source_rows() -> list[dict]:
    ...
    if not rows:
        raise RuntimeError("No publishing_queue rows found in notion sync preview.")
    ...
    for row in rows:
        ...
        normalized_rows.append({
            ...
            "release_version": props.get("release_version"),
            "public_url": props.get("public_url"),
            "unlisted_url": props.get("unlisted_url"),
            "visibility": props.get("visibility"),
            "public_status": props.get("public_status"),
            "upload_ready": props.get("upload_ready"),
            "first_public_video": props.get("first_public_video"),
            "next_step": props.get("next_step"),
            "source_record": props.get("source_record"),
            "video_file_path": props.get("video_file_path"),
            "thumbnail_image_path": props.get("thumbnail_image_path")
        })
...
def build_properties_payload(row: dict, field_mapping: dict) -> dict:
    ...
    title_mapping = mapped["queue_item_title"]
    title_property_name = title_mapping["notion_property_name"]
    payload[title_property_name] = notion_title(row.get("queue_item_title"))

    for source_field, mapping in mapped.items():
        if source_field == "queue_item_title":
            continue
        ...
        elif notion_property_type == "status":
            text = str(value or "").strip()
            if text:
                payload[notion_property_name] = {"status": {"name": text}}
...
def build_planned_operations(rows: list[dict], field_mapping: dict) -> list[dict]:
    ...
    for row in rows:
        ...
        operations.append({
            "key": row["key"],
            "queue_item_title": row["queue_item_title"],
...
def main() -> None:
    ...
    if args.patch_schema:
        ...
        if schema_patch["status"] in {"schema_patch_failed", "schema_patch_partial"}:
            ...
            record = {
                "record_type": "notion_publishing_queue_sync",
                "version": "1.0",
                ...
                "summary": {
                    ...
                    "next_step": "fix_publishing_queue_schema_patch"
                },
                "target": {
            ...
            print(f"Notion Publishing Queue Sync failed during schema patch: {status}")
            return
    ...
    if args.apply and failed_apply_results:
        status = "sync_failed"
        next_step = "inspect_failed_publishing_queue_sync"
    elif args.apply:
        status = "sync_passed"
        next_step = "review_publishing_queue_in_notion_then_build_os_sync_runner"
    elif args.patch_schema:
        status = schema_patch["status"] if schema_patch else "schema_patch_not_needed"
        next_step = "run_publishing_queue_sync_dry_run"
    else:
        status = "dry_run_ready"
        next_step = "run_publishing_queue_apply_sync"
    ...
    print("Notion Publishing Queue Sync completed.")
